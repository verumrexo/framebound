...
test('sound editor explains immediate staging versus source promotion', () => {
    assert.match(PART_SOUND_EDITOR_INTRO, /only this part's sound slots are shown/);
    assert.match(PART_SOUND_EDITOR_INTRO, /duplicate before shaping/);
    assert.match(PART_SOUND_EDITOR_INTRO, /save sound stores audio/);
    assert.match(PART_SOUND_EDITOR_INTRO, /save part commits slot assignments/);
